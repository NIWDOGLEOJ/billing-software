...
import { Camera, RefreshCw, Zap, ZapOff, Image as ImageIcon, X, AlertCircle } from 'lucide-react';
import { captureVideoFrameToDataUrl, compressImageFileToDataUrl } from '../utils/imageCompressor';
...
export const ProductPhotoCaptureModal: React.FC<ProductPhotoCaptureModalProps> = ({
  ...
  const startCamera = async (mode: 'environment' | 'user') => {
    ...
    if (!hasGUM) {
      setCameraError('Live in-app camera is not supported in this browser context (e.g. plain HTTP LAN). Please use the photo picker below.');
  ...
  // closes, restart the in-app stream only if it was lost (e.g. the OS killed
  // the MediaStream track while the app was backgrounded). Never close the modal
  // here — that is what caused the "exits automatically" bug.
  useEffect(() => {
    if (!isOpen) return;
    ...
    const handleVisibilityChange = () => {
      ...
      // Page became visible again (user returned from native camera/gallery).
      // Clear the native-picker flag. If the OS killed our stream while we
      // were in the background, restart it.
  ...
  // Keyboard shortcut listener (Escape to close, Space to snap).
  // Guard against spurious Escape events fired on Android when the native
  // camera/picker UI is dismissed — those must not close the modal.
  useEffect(() => {
    if (!isOpen) return;
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (e.key === 'Escape') {
        e.preventDefault();
        handleClose();
      } else if (e.key === ' ' && stream && !isCapturing) {
        e.preventDefault();
        handleSnapPhoto();
      }
    };
  ...
  const handleFileInput = async (e: React.ChangeEvent<HTMLInputElement>) => {
    // Native picker just closed (with or without a file) — clear the flag.
    nativePickerActiveRef.current = false;
                ...
                <div className="flex flex-col items-center gap-3.5">
                  ...
                  <div>
                    <h4 className="text-sm font-bold text-white mb-1">
                      {cameraError?.includes('denied') ? 'Camera Access Required' : 'Live Camera Unavailable'}
                    </h4>
                  ...
                  <div className="flex flex-col gap-2 w-full pt-1">
                    <button
                      ...
                      className="w-full h-10 px-4 rounded-xl text-xs font-bold flex items-center justify-center gap-2 cursor-pointer transition-transform active:scale-95 shadow-md"
