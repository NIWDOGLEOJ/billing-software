...
export function ThemeProvider({ children }: { children: ReactNode }) {
  ...
  const [activePresetId, setActivePresetId] = useState<string>(() => {
    return safeGetStorage('themePresetId') || 'emerald';
  });

  const [accentColor, setAccentColorState] = useState<string>(() => {
    const saved = safeGetStorage('themeAccentColor');
    if (saved) return saved;
    const preset = COLOR_THEME_PRESETS.find(p => p.id === (safeGetStorage('themePresetId') || 'emerald'));
    if (preset) {
      return getSystemTheme() === 'dark' ? preset.accentDark : preset.accentLight;
    }
  ...
  const [surfaceColor, setSurfaceColorState] = useState(() => {
    return safeGetStorage('themeSurfaceColor') || DEFAULT_SURFACE_COLOR;
  });
  ...
  useEffect(() => {
    safeSetStorage('themeAccentColor', accentColor);
    safeSetStorage('themeSurfaceColor', surfaceColor);
    ...
    if (surfaceColor) {
      ...
      safeRemoveStorage('themeSurfaceColor');
  ...
  const setAccentColor = (color: string) => {
    setActivePresetId('');
    safeRemoveStorage('themePresetId');
  ...
  const resetThemeDefaults = () => {
    ...
    safeSetStorage('themePresetId', 'emerald');
