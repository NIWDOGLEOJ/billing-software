...
import { ShoppingBag, Eye, Package, Sparkles } from 'lucide-react';
...
export const ProductCard: React.FC<ProductCardProps> = ({ product, onQuickView }) => {
  ...
  return (
    <div className="group relative bg-white rounded-2xl border border-slate-200/80 hover:border-emerald-300 shadow-xs hover:shadow-lg hover:shadow-emerald-900/5 transition-all duration-300 flex flex-col overflow-hidden">
      {/* Top Image & Badge Container */}
      <div className="relative w-full pt-[75%] bg-slate-100 overflow-hidden cursor-pointer" onClick={() => onQuickView(product)}>
        {/* Popular Tag */}
        {product.isPopular && (
      ...
      <div className="p-4 flex-1 flex flex-col justify-between">
        <div>
          ...
          <h3
            ...
            className="text-sm font-bold text-slate-800 group-hover:text-emerald-700 transition-colors line-clamp-2 cursor-pointer leading-snug"
            title={product.name}
          >
            {product.name}
          </h3>
          ...
          <div className="mt-3">
            {isOutOfStock ? (
              ...
              <button
                ...
                className="w-full bg-slate-900 hover:bg-emerald-600 active:bg-emerald-700 text-white text-xs font-bold py-2 px-3 rounded-xl transition-all flex items-center justify-center gap-1.5 shadow-xs cursor-pointer"
              >
