...
router.post('/', authenticateToken, (req: AuthRequest, res: Response) => {
  ...
  const transaction = db.transaction(() => {
    ...
    `).run(
      ...
      Number(total),
      payment_mode || 'cash',
      amount_received !== undefined ? Number(amount_received) : null,
      change_amount !== undefined ? Number(change_amount) : null,
      Number(rounding_adjustment || 0),
      Number(points_earned || 0),
      Number(points_redeemed || 0),
      JSON.stringify(items),
      shop_details ? JSON.stringify(shop_details) : null,
      gst_enabled ? 1 : 0,
      Number(gst_rate || 0),
      customer_gstin || null,
      pricing_tier || 'retail'
    ...
    for (const item of items) {
      ...
      `).run(
        ...
        'sale',
        bill_number,
        `Sold via bill #${bill_number}${item.selectedBatch ? ` (Batch: ${item.selectedBatch})` : ''}`,
        date || new Date().toISOString()
      );
    }
    ...
    if (customer_phone) {
      const existingCust = db.prepare('SELECT * FROM customers WHERE phone = ?').get(customer_phone) as any;
  ...
  try {
    ...
    const createdBill = db.prepare('SELECT * FROM bills WHERE id = ?').get(id) as any;
    if (createdBill) {
