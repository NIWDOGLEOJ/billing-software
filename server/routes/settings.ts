...
router.post('/restore', authenticateToken, requireOwner, (req: AuthRequest, res: Response) => {
  ...
  const transaction = db.transaction(() => {
    ...
    const restoreTable = (tableName: string, dataArray: any[], columns: string[]) => {
...
router.post('/install', authenticateToken, (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    let activeShiftRow: any = null;

    if (callingUser) {
      try {
        userRow = db.prepare('SELECT * FROM users WHERE id = ?').get(callingUser.id);
      } catch (e) { console.warn('[INSTALL] Could not snapshot user:', e); }
      try {
        sessionRow = db.prepare(
          'SELECT * FROM login_sessions WHERE id = ? AND logout_time IS NULL'
        ).get(callingUser.sessionId);
      } catch (e) { console.warn('[INSTALL] Could not snapshot session:', e); }
      try {
        activeShiftRow = db.prepare(
          "SELECT * FROM shift_records WHERE user_id = ? AND status = 'active'"
        ).get(callingUser.id);
      } catch (e) {}
    }
    ...
    if (userRow) {
      try {
        ...
        if (!existing) {
          db.prepare(
            'INSERT INTO users (id, username, email, name, role, password_hash, permissions, phone, is_active, created_at) VALUES (?,?,?,?,?,?,?,?,?,?)'
          ).run(
            userRow.id, userRow.username, userRow.email, userRow.name,
            userRow.role, userRow.password_hash, userRow.permissions,
            userRow.phone, userRow.is_active, userRow.created_at
          );
          console.log(`[INSTALL] Migrated user ${userRow.username} into ${sector}.db`);
        }
      } catch (e) { console.warn('[INSTALL] User migration failed (may already exist):', e); }
    }
    ...
    if (sessionRow) {
      try {
        ...
        if (!existingSession) {
          db.prepare(
            'INSERT INTO login_sessions (id, user_id, login_time, logout_time, duration, last_active_at, device_type, is_attendance) VALUES (?,?,?,?,?,?,?,?)'
          ).run(
            sessionRow.id, sessionRow.user_id, sessionRow.login_time,
            sessionRow.logout_time ?? null, sessionRow.duration ?? null,
            sessionRow.last_active_at ?? null, sessionRow.device_type ?? 'desktop',
            sessionRow.is_attendance ?? 1
          );
          console.log(`[INSTALL] Migrated active session ${sessionRow.id} into ${sector}.db`);
        }
      } catch (e) { console.warn('[INSTALL] Session migration failed:', e); }
    }
    ...
    if (activeShiftRow) {
      try {
        ...
        if (!existingShift) {
          db.prepare(
            'INSERT INTO shift_records (id, user_id, user_name, start_time, end_time, initial_cash, system_cash, system_upi, system_card, actual_cash, actual_upi, actual_card, discrepancy_cash, status, notes) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)'
          ).run(
            activeShiftRow.id, activeShiftRow.user_id, activeShiftRow.user_name,
            activeShiftRow.start_time, activeShiftRow.end_time ?? null,
            activeShiftRow.initial_cash, activeShiftRow.system_cash,
            activeShiftRow.system_upi, activeShiftRow.system_card,
            activeShiftRow.actual_cash ?? null, activeShiftRow.actual_upi ?? null,
            activeShiftRow.actual_card ?? null, activeShiftRow.discrepancy_cash,
