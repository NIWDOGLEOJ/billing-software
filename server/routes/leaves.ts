...
router.post('/', authenticateToken, requireOwner, (req: AuthRequest, res) => {
  ...
  try {
    ...
    const existing = db.prepare('SELECT COUNT(*) as c FROM leaves WHERE date = ? AND (user_id = ? OR (user_id = \'all\' AND ? = \'holiday\'))')
      .get(date, user_id || 'all', type) as { c: number };
    ...
    `).run(
      id,
      user_id || 'all', // Default 'all' represents a store-wide holiday
      user_name || 'Store Wide',
      date, // YYYY-MM-DD
