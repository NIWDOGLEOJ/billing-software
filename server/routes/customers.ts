...
router.put('/:phone', authenticateToken, (req: AuthRequest, res: Response) => {
  const { phone } = req.params;
  ...
  try {
    ...
    if (!customer) {
      ...
      `).run(
        ...
        loyalty_points !== undefined ? Number(loyalty_points) : customer.loyalty_points,
        total_spent !== undefined ? Number(total_spent) : customer.total_spent,
        visit_count !== undefined ? Number(visit_count) : customer.visit_count,
        new Date().toISOString(),
        phone
      );
    }
...
router.post('/:phone/pay-balance', authenticateToken, (req: AuthRequest, res: Response) => {
  const { phone } = req.params;
  ...
  try {
    ...
    `).run(
      ...
      Number(amount),
      0, 0, 0, 0,
      JSON.stringify([{ id: 'ledger_payment', name: 'Ledger Balance Settle Payment', price: Number(amount), quantity: 1 }]),
      null, 0, 0
    );
    ...
    if (broadcast) {
      ...
      const payBill = db.prepare('SELECT * FROM bills WHERE id = ?').get(billId) as any;
      if (payBill) {
