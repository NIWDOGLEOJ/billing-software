...
export function initDb(sector?: string) {
  ...
  if (prodCount === 0) {
    ...
    const sectorProducts: Record<string, any[][]> = {
      retail: [
    ...
    for (const p of items) ins.run(...p);
  ...
  if (whCount === 0) {
    ...
    const sectorWarehouses: Record<string, any[][]> = {
      retail: [
  ...
  const sectorSettings: Record<string, [string, string][]> = {
    retail: [
  ...
  for (const [k, v] of settingsToSeed) insSettings.run(k, v);
