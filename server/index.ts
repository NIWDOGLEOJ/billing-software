...
function broadcastActiveUsers() {
  const uniqueUsers: Array<{ name: string; role: string; id: string | number }> = [];
      ...
      if (data.type === 'ping') {
        ...
        try {
          ...
          ins.run(
            ...
            chatMsg.timestamp,
            chatMsg.fingerprint,
            chatMsg.recipientName || 'All',
            chatMsg.isBillTransfer ? 1 : 0
          );
        } catch (dbErr: any) {
...
function runChatMaintenance() {
  ...
  try {
    ...
    if (!fs.existsSync(archivePath)) {
      ...
      const yesterdayChats = db.prepare(
        'SELECT * FROM chats WHERE timestamp >= ? AND timestamp <= ? ORDER BY timestamp ASC'
      ).all(yesterdayStart, yesterdayEnd) as any[];
