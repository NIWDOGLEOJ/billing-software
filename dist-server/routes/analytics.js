...
router.get('/sales', authenticateToken, (req, res) => {
    ...
    try {
        const bills = db.prepare('SELECT total, date FROM bills ORDER BY date ASC').all();
...
router.get('/employee-performance', authenticateToken, requireOwner, (req, res) => {
    try {
        const bills = db.prepare("SELECT cashier_id, cashier_name, total FROM bills WHERE cashier_id != 'dev_1'").all();
        const sessions = db.prepare("SELECT user_id, duration FROM login_sessions WHERE user_id != 'dev_1'").all();
        const breaks = db.prepare("SELECT user_id, duration FROM break_records WHERE duration IS NOT NULL AND user_id != 'dev_1'").all();
        const users = db.prepare("SELECT id, name, role FROM users WHERE username != 'developer' AND id != 'dev_1'").all();
        const statsMap = {};
