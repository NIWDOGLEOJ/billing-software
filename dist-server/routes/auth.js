...
router.post('/login', (req, res) => {
    ...
    try {
        ...
        try {
            const activeSessions = db.prepare('SELECT id, login_time FROM login_sessions WHERE user_id = ? AND logout_time IS NULL').all(user.id);
...
router.get('/session/:id', authenticateToken, (req, res) => {
    try {
        const session = db.prepare('SELECT * FROM login_sessions WHERE id = ?').get(req.params.id);
        if (!session) {
