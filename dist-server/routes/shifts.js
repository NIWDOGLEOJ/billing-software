...
router.post('/end', authenticateToken, (req, res) => {
    ...
    try {
        ...
        const bills = db.prepare('SELECT total, payment_mode FROM bills WHERE cashier_id = ? AND date >= ?').all(req.user.id, activeShift.start_time);
        let systemCash = 0;
        let systemUpi = 0;
        let systemCard = 0;
        for (const bill of bills) {
            ...
            if (mode === 'cash')
                systemCash += bill.total;
            else if (mode === 'upi')
                systemUpi += bill.total;
            else if (mode === 'card')
                systemCard += bill.total;
        }
      ...
      SET end_time = ?, system_cash = ?, system_upi = ?, system_card = ?,
          ...
          status = 'closed', notes = ?
      WHERE id = ?
    `).run(endTime, systemCash, systemUpi, systemCard, Number(actualCash), Number(actualUpi), Number(actualCard), discrepancyCash, notes || '', activeShift.id);
