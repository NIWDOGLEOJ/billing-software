...
router.post('/', authenticateToken, requirePermission('access_inventory'), (req, res) => {
    ...
    try {
      ...
      `).run(Number(stock_quantity), expiry_date, manufacturing_date || '', drug_license || null, Number(prescription_required || 0), product_id, batch_number);
        }
        else {
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
      `).run(id, product_id, batch_number, expiry_date, manufacturing_date || '', Number(stock_quantity), drug_license || null, Number(prescription_required || 0), now);
        }
...
router.put('/:id', authenticateToken, requirePermission('access_inventory'), (req, res) => {
    ...
    try {
        const batch = db.prepare('SELECT * FROM medicine_batches WHERE id = ?').get(id);
        if (!batch) {
      ...
      WHERE id = ?
    `).run(Number(stock_quantity), expiry_date || batch.expiry_date, manufacturing_date || batch.manufacturing_date, drug_license || batch.drug_license, Number(prescription_required !== undefined ? prescription_required : batch.prescription_required), id);
        if (diff !== 0) {
