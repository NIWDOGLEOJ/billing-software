...
router.get('/', authenticateToken, (req, res) => {
    try {
        ...
        const chats = db.prepare('SELECT * FROM chats WHERE timestamp >= ? ORDER BY timestamp ASC').all(cutOffTime);
...
router.get('/developer', authenticateToken, (req, res) => {
    try {
...
router.get('/senders', authenticateToken, (req, res) => {
    try {
        if (req.user?.username !== 'developer') {
            return res.status(403).json({ error: 'Developer access required' });
        }
        const senders = db.prepare('SELECT DISTINCT sender_name FROM chats ORDER BY sender_name ASC').all();
        res.json(senders.map((s) => s.sender_name));
    }
    catch (error) {
        res.status(500).json({ error: error.message });
    }
