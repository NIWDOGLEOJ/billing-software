...
router.get('/', authenticateToken, requireOwner, (req, res) => {
    try {
        const users = db.prepare('SELECT id, username, email, name, role, permissions, phone, is_active, created_at FROM users').all();
...
router.get('/breaks', authenticateToken, (req, res) => {
    try {
        const breaks = db.prepare('SELECT b.*, u.name as user_name FROM break_records b JOIN users u ON b.user_id = u.id ORDER BY b.start_time DESC').all();
        res.json(breaks);
    }
    catch (error) {
        res.status(500).json({ error: error.message });
...
router.get('/sessions', authenticateToken, requireOwner, (req, res) => {
    try {
        const sessions = db.prepare('SELECT s.*, u.name as user_name, u.role as user_role FROM login_sessions s JOIN users u ON s.user_id = u.id ORDER BY s.login_time DESC').all();
        res.json(sessions);
    }
    catch (error) {
        res.status(500).json({ error: error.message });
