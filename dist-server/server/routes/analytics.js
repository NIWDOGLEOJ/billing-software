...
const router = Router();
// GET /api/analytics/summary (requires owner/co-owner or view_analytics permission)
...
router.get('/employee-performance', authenticateToken, requireOwner, (req, res) => {
    try {
        const bills = db.prepare('SELECT cashier_id, cashier_name, total FROM bills').all();
        const sessions = db.prepare('SELECT user_id, duration FROM login_sessions').all();
        const breaks = db.prepare('SELECT user_id, duration FROM break_records WHERE duration IS NOT NULL').all();
